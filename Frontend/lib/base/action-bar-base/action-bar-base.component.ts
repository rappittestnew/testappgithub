...
export class ActionBarBaseComponent {
  ...
  @Input() filterActionButtons: any = {  // Filter Actions hide
    hideActions: []
  }
  ...
  evaluateButtonRights(btn: any, selected: boolean) {
    if (btn) {
      if (btn.type === ActionBar.BUTTON && btn.enableOnlyIfRecordSelected && (this.conditionBasedButtons?.disableButtons?.indexOf(btn.buttonId === -1) || this.conditionBasedButtons?.disableButtons.length === 0)) {
        this.conditionBasedButtons?.disableButtons?.push(btn.buttonId);
      }
      if (btn.type === ActionBar.BUTTONGROUP) {
        btn.children?.forEach((o: any) => {
          if (o.enableOnlyIfRecordSelected && (this.conditionBasedButtons?.disableButtons?.indexOf(o.buttonId === -1) || this.conditionBasedButtons?.disableButtons.length === 0))
            this.conditionBasedButtons?.disableButtons?.push(o.buttonId);
        })
      }
      if (selected) {
  ...
  updateDropdownActions(arr: any) {
    return arr.map((item: any) => {
      return {
        ...
        disabled: (item.buttonEnabled == 'no' || this.conditionBasedButtons.disableButtons.includes(item?.buttonId)) || this.detailConditionalButtons.disableActions.includes(item.buttonId),
        iconStyle: { color: '', },
        style: {
  ...
  hideActions(btn: any) {
    return ((btn.visibility === 'hide' || btn.visible === false) ||
      ...
      || this.detailConditionalButtons.hideActions.includes(btn.buttonId)
      || this.filterActionButtons.hideActions.includes(btn.buttonId))
